...
resource "aws_synthetics_canary" "clientapp-website" {
  name                 = "clientapp"
  artifact_s3_location = "s3://${aws_s3_bucket.cw_canary_bucket.id}/clientapp"
  execution_role_arn   = aws_iam_role.cw_canary_iam_role.arn 
  handler              = "clientapp.handler"
  zip_file             = "clientapp/clientappv1.zip"
...
# AWS CloudWatch Metric Alarm for Synthetics Heart Beat Monitor
resource "aws_cloudwatch_metric_alarm" "synthetics_alarm_clientapp" {
  alarm_name          = "Synthetics-Alarm-Clientapp"
  ...
  dimensions = {
    CanaryName = aws_synthetics_canary.clientapp-website.id
  }
